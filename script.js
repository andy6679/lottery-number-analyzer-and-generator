...
class UKLotteryGenerator {
  constructor() {
    this.mainMin = 1;
    this.mainMax = 59;  // Adjusted for UK lottery
    this.totalMainNumbers = 6;  // UK Lottery has 6 main numbers
  }
      ...
      while (!valid && attempts < maxAttempts) {
        ...
        if (valid) {
          ...
          mainNumbers.forEach(n => {
            if (n <= 19) lowCount++;       // Low range 1-19
            else if (n <= 39) midCount++; // Mid range 20-39
            else highCount++;             // High range 40-59
          });
  ...
  formatResult(result) {
    const { mainNumbers, stats } = result;
    
    const formattedMain = mainNumbers.map(n => {
      let className = 'number-ball';
      if (n <= 19) className += ' low-number';
      else if (n <= 39) className += ' mid-number';
...
// Example usage:
const generator = new UKLotteryGenerator();
